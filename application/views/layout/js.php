<script src="<?php echo base_url() ?>/asset/js/jquery-3.3.1.min.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/jquery-migrate-3.0.1.min.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/jquery-ui.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/popper.min.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/bootstrap.min.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/owl.carousel.min.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/jquery.stellar.min.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/jquery.countdown.min.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/bootstrap-datepicker.min.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/jquery.easing.1.3.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/aos.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/jquery.fancybox.min.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/jquery.sticky.js"></script>
  <script src="<?php echo base_url() ?>/asset/js/jquery.mb.YTPlayer.min.js"></script>




  <script src="<?php echo base_url() ?>/asset/js/main.js"></script>